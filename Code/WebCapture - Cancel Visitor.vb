...
Module Script
	Sub Form_OnLoad(ByVal eventData As ClientEventData)
		...
		eventData.Form.Fields.GetField("ConfirmCancel").IsHidden = True
	...
	Sub Visitors_OnChange(ByVal eventData As ClientEventData)
		...
		vsSelected = "C:\Nuance\CompleteDemo\Settings\BookedVisits\" & eventData.User.UserName & "\" &  vsList.Value & ".txt"
		...
		If (File.Exists(vsSelected)) Then
			...
			Else
				...
				eventData.Form.Fields.GetField("VisitCarDetails").Value = vsCarReg				
			End If
			eventData.Form.Fields.GetField("ConfirmCancel").IsHidden = False
