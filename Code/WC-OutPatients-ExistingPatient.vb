...
Module Script
    Sub Form_OnLoad(ByVal eventData As MFPEventData)
		...
		Dim DocumentTypeTextFile As String = "C:\Nuance\CompleteDemo\Settings\PatientRecords\DocumentList.txt"
		...
		Dim ConsultantTextFile As String = "C:\Nuance\CompleteDemo\Settings\PatientRecords\ConsultantList.txt"
	...
	Sub Search_OnChange(ByVal eventData As MFPEventData)
		...
		If eventData.Form.Fields.GetField("Search").Value = True Then

			' Search based on entries

			Dim patientDir As String = \\AutoStore7\PatientRecords"
			...
			eventData.Form.Fields.GetField("Search").Value = False
	...
	Sub SearchAgain_OnChange(ByVal eventData As MFPEventData)

		If eventData.Form.Fields.GetField("SearchAgain").Value = True Then
			...
			eventData.Form.Fields.GetField("Patients").isHidden = True
			eventData.Form.Fields.GetField("SearchAgain").isHidden = True
			eventData.Form.Fields.GetField("SearchAgain").Value = False
	...
	Sub Patients_OnChange(ByVal eventData As MFPEventData)
