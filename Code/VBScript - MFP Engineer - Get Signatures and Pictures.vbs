Sub GetSignatures_OnLoad
	...
	While Not(KnowledgeDocument Is Nothing)
		...
		If ucase(left(FileName,18)) = "ENGINEERSIGNATURE_" Then
			EngineerSignatureFile = FilePath
		...
		If ucase(left(FileName,18)) = "CUSTOMERSIGNATURE_" Then
			CustomerSignatureFile = FilePath
			EKOManager.StatusMessage "New Signature File Path: " & FilePath 
			KnowledgeDocument.Visible = False
			FileCount = FileCount - 1
		end if
		
		If  ucase(left(FileName,18)) <> "ENGINEERSIGNATURE_" And ucase(left(FileName,18)) <> "CUSTOMERSIGNATURE_" Then
	...
	If Not(PTopic Is Nothing) Then
		PTopic.Replace "~USR::EngineerSignature~", EngineerSignatureFile
